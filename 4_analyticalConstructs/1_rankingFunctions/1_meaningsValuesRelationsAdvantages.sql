...
SELECT first_name, last_name, hire_date, ROW_NUMBER() OVER(ORDER BY hire_date ASC) FROM analytical_cons_ranking_functions.employees;

-- 2 Basic RANK()
-- Rank employees based on their salary in descending order. Show how ties are handled (gaps in rank).
SELECT first_name, last_name, salary, RANK() OVER(ORDER BY salary DESC) FROM analytical_cons_ranking_functions.employees;
...
SELECT first_name, last_name, salary, DENSE_RANK() OVER(ORDER BY salary DESC) FROM analytical_cons_ranking_functions.employees;
...
SELECT department, salary, first_name, last_name, rn
FROM ( SELECT *, ROW_NUMBER() OVER(PARTITION BY department ORDER BY salary DESC) rn
		FROM analytical_cons_ranking_functions.employees ) AS subquery
WHERE rn IN (1, 2)
